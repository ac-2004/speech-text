...
public class TranscriptionControllerTest {
    ...
    // Lets us simulate HTTP requests to the controller without launching a real web server.
    @Autowired
    ...
    void receiveAudio_returnsStubbedTranscription() throws Exception {
        ...
        when(transcriptionService.transcribe(any(AudioData.class)))
                .thenReturn(Mono.just("hello from stub"));
    ...
    @Test
    void transcribe_whenServiceFails_returnsStandard500Error()
            throws Exception {
        ...
        when(transcriptionService.transcribe(any(AudioData.class)))
                .thenReturn(Mono.error(
                        new RuntimeException("simulated transcription failure")
                ));
